...
// Si han pasado 7 días desde la última actualización, muestra una alerta
  ...
  .then(data => {
    ...
    let selectedRows = [];
    let selectedRows2 = [];
    const nombreFiltrar = "DANIELA CEJAS"; // Nombre específico para filtrar

    const filteredRows = rows.filter(row => row[2]?.toUpperCase() === nombreFiltrar);
    console.log(filteredRows);
    if (permiso === "super usuario") {
      selectedRows = filteredRows.map((row, index) => {
      ...
      selectedRows2 = filteredRows.map((row, index) => {
        // Añade la fila visible en la hoja (índice + 1)
        const numeroFila = index + 1;
        let isAtrasado = 0; // Indicador de atraso (0: no atrasado, 1: atrasado)
        const lastUpdateDate = row[55] || "";
